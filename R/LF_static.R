                       ...
                       get_likelihood = function(task, only_observed = FALSE){
                         ...
                         if(only_observed){
                           ...
                           if(outcome_type$type == "binomial"){
                             ...
                             level_mat <- matrix(levels, nrow=length(values), ncol=length(levels), byrow=T)
                             likelihood <- apply(level_mat, 2, function(level_vec) as.numeric(level_vec==values))
                           } else{
                             stop("currently, only binomial likelihoods are suppported")
                           }
                         }
