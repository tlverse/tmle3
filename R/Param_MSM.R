  ...
  public = list(
    ...
    clever_covariates = function(tmle_task = NULL, fold_number = "full") {
      ...
      msm_terms <- self$msm_terms %>%
        str_replace_all(self$treatment_node, "A") %>%
        str_replace_all(self$strata_name, "V")
    ...
    estimates = function(tmle_task = NULL, fold_number = "full") {
      ...
      msm_terms <- self$msm_terms %>%
        str_replace_all(self$treatment_node, "A_ext") %>%
        str_replace_all(self$strata_name, "V_ext")
      ...
      H2A <- split(
        as.data.table(weighted_res * phi_ext),
