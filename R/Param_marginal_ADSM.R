#' Adaptive Design Specific Mean (Marginalized Over Covaraite Distribution)
#'
#' Parameter definition for the Adaptive Design Specific Mean (ADSM) (Marginaled Over Covaraite Distribution): $P_{0,W}[E_{g_adapt}(Y|A=a|W)]$. Currently supports adaptive design for BINARY intervention nodes.
...
#' }
#' @export
Param_marginal_ADSM <- R6Class(
  classname = "Param_marginal_ADSM",
  portable = TRUE,
  class = TRUE,
  inherit = Param_base,
  public = list(
  ...
  active = list(
    name = function() {
      ...
      param_form <- "Marginal_ADSM[Y_{g_adapt}]"
      return(param_form)
    },
    g_treat = function(){
