...
fit_tmle_likelihood <- function(likelihood, task, param){
  ...
  # tmle_fluc <- make_learner(Lrnr_optim, submodel_logit, loss_loglik_binomial, init_0 = TRUE)
  
  
  #make pipe that generates fluctuated predictions
  #wrap likelihood in custom chain that defines quantities relevant for fluctuation
  fluc_chain <- function(likelihood, task){
    ...
    fluc_task <- sl3_Task$new(tmle_data, outcome="Y", offset="EY", covariates="HA")
