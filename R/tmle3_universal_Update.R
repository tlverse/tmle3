...
#' @export
#
tmle3_universal_Update <- R6Class(
  classname = "tmle3_universal_Update",
  portable = TRUE,
  class = TRUE,
  public = list(
    initialize = function(maxit = 10000, delta_epsilon = 1e-5, line_search = FALSE) {
      private$.maxit <- maxit
      private$.delta_epsilon <- delta_epsilon
      private$.line_search <- line_search
    },
    ...
    generate_submodel_data = function(likelihood, tmle_task, cv_fold = -1) {
      ...
      estimates <- lapply(self$tmle_params, function(tmle_param) tmle_param$estimates(tmle_task, cv_fold))
