...
```{r}
task1 <- task$clone()
task1$force_at_risk <- T
tlik$sync_task(task1)
data.table(tlik$get_likelihood(task1, "Y"))
data.table(tlik$get_likelihood(task, "Y"))

data.table(likelihood$get_likelihood(task1, "Y"))
data.table(likelihood$get_likelihood(task, "Y"))
param$gradient$expand_task(task, "Y")$uuid
param$gradient$expand_task(task1, "Y")$uuid
