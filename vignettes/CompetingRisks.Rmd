...
```{r}
param = Param_CR$new(likelihood, list("A" = LF_static$new("A", value = 1)), competing_risk_nodes = c("processN2"), target_risk_node = "processN1")
param$name
...
H = param$clever_covariates(task, "full")
```

```{r}
cf_task <- task$clone()
cf_task$force_at_risk <- T
H = param$clever_covariates(cf_task, "full")
