...
grad <- Gradient$new(lik, generator, list(update_nodes = "Y"))
...
plot(gen_task$X$W[keep], ipws[keep])
plot(gen_task$X$W[keep], ipw_pred[keep])
keep <- (ipws > 0)
plot( ipws[keep], ipw_pred[keep])
plot(gen_task$X$W[keep], ipws[keep])
plot(gen_task$X$W[keep], ipw_pred[keep])
...
true <- lapply(data[5], as.data.table)[[1]]
