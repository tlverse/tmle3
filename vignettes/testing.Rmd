...
learner_list <- list(
  ...
  Y = Lrnr_stratified$new(Lrnr_glmnet$new(formula = ~.^2, family = "poisson"), variable_stratify = c("A_binary"))
 
)
 
spec <- tmle3_Spec_contglm$new(~1+W, ~1)
fit <- tmle3(spec, data, node_list, learner_list)
 
psi <-  fit$summary$tmle_est
se <- fit$summary$se
ci <-  cbind(ci, psi - 1.96 * se   <= c(0,1,1) &  psi + 1.96 * se   >= c(0,1,1))
...
spec <- tmle3_Spec_contglm$new(~1+W, ~1, fA = function(x){ log(x)}, dfA = function(x){1/x}, submodel = "poisson")
fit <- tmle3(spec, data, node_list, learner_list)
 print(fit$summary)
psi <-  fit$summary$tmle_est
se <- fit$summary$se
