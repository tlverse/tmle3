...
survival_spec <- tmle3_Spec_coxph$new(formula = ~ 1 + as.numeric(karno),
  treatment_level = 1, control_level = 0
)

learner_list <- list(A_c = Lrnr_glmnet$new(), A = Lrnr_glmnet$new(), N = Lrnr_glmnet$new())
...
spec_spCATE <- tmle3_Spec_npCausalGLM$new(~1 + W, "CATT", family = "binomial")
 (out <- tmle3(spec_spCATE, data, node_list, learner_list = learner_list) )
 out <- out$summary
