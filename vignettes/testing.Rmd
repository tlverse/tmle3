...
tmle_task <- point_tx_task(data, node_list, scale_outcome = F, include_variance_node = TRUE)
...
updater <- tmle3_Update$new(verbose = T, bounds = list(Y = c(-Inf, Inf)))
tlik <- Targeted_Likelihood$new(likelihood, updater = updater)
param_cate <- Param_spCATE$new(tlik, formula_CATE  = ~1, intervention_list_treatment = list(define_lf(LF_static, "A", value = 1)), intervention_list_control = list(define_lf(LF_static, "A", value = 0)))
...
```{r}
param_cate$estimates(tmle_task)$psi
sd(param_cate$estimates(tmle_task)$IC)
```
