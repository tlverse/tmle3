...
```{r}
tsk = task[c(1,1,1)]
newcols = tsk$add_columns(data.table(id = c(3,2,4))) 
tsk = tsk$next_in_chain(column_names = newcols)
tsk$data
```
...
```{r}
n=100
way = data.table(W = runif(n), A = rbinom(n, size = 1, 0.5), Y = rbinom(n, size =1, 0.5), Delta =rbinom(n, size = 1, 0.5) )
way[way$Delta==0, Y:= NA]
way
```

```{r}
library(data.table)
npsem = list(define_node("W", c()), define_node("Y", c("Y"), c("A")),  define_node("A", c("A")))
...
```{r}
task$npsem$Y
task$get_tmle_node("delta_Y", include_id = T, include_time = T, expand = T, compute_risk_set = F)
...
```{r}
library(sl3)
likelihood_factors <- list("W" = LF_emp$new("W" ), "A" = LF_fit$new("A", make_learner(Lrnr_mean)), "Y" =  LF_fit$new("Y", make_learner(Lrnr_glm), type = "mean"))
...
liks$factor_list$A$get_likelihood(task, "full")
liks$get_likelihood(task, "A")
liks$get_likelihoods(task)
```
