...
```{r}
data.table(A = data.table(A = 1:10))
...
```{r}
setDT(unlist(list(data.table(NULL), data.table(1:10)), recursive = F))[]
...
task$get_tmle_node("Delta", format = F)
task$get_tmle_node("Y", format = F)
...
```{r}
library(sl3)
likelihood_factors <- list("W" = LF_emp$new("W" ), "A" = LF_fit$new("A", make_learner(Lrnr_glm)), "Y" =  LF_fit$new("Y", make_learner(Lrnr_glm), type = "mean"))
