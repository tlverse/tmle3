...
```{r}
data.table(matrix(tlik$get_likelihood(cf_task, "processN"), nrow = 1000, byrow = T))
lik_trained$get_likelihood(cf_task, "processN", drop_id = F, to_wide = T)
task$get_tmle_node("processN", include_id = T)
```

```{r}
tlik <- Targeted_Likelihood$new(lik_trained)
...
```{r}
tlik$updater$generate_submodel_data(tlik, cf_task, update_node = "processN")$initial
#tlik$updater$update_step(tlik, task)
